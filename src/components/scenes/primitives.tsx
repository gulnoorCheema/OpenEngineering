...
export function Bolt({
  ...
  return (
    <mesh position={position} rotation={rotation} castShadow>
      ...
      <meshStandardMaterial color="#52616d" metalness={0.8} roughness={0.3} />
    </mesh>
  );
}
export function Gear({
  ...
  const geometry = useMemo(() => {
    ...
    for (let tooth = 0; tooth < teeth; tooth++) {
      ...
      points.forEach(([f, r], i) => {
        const a = ((tooth + f) / teeth) * Math.PI * 2;
        if (tooth === 0 && i === 0) shape.moveTo(Math.cos(a) * r, Math.sin(a) * r);
  ...
  return (
    <group position={position} rotation={[0, 0, angle]}>
      ...
      <mesh position={[0, 0, depth / 2 + 0.06]}>
        <torusGeometry args={[radius * 0.5, 0.035, 8, 60]} />
        <meshStandardMaterial color="#fff0d9" metalness={0.6} roughness={0.3} />
      </mesh>
      <mesh position={[radius * 0.69, 0, depth / 2 + 0.03]}>
