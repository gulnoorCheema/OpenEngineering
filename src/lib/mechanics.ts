...
export function engineKinematics(phase: number, cylinder = 0, cylinders = 1) {
  if (!Number.isFinite(phase) || ![1,4].includes(cylinders) || !Number.isInteger(cylinder) || cylinder < 0 || cylinder >= cylinders) throw new RangeError('Invalid engine phase or cylinder');
  ...
  return { angle, cycle, crankX, crankY, pistonY, radius, rodLength,
    stroke: STROKES[strokeIndex], strokeIndex, strokePhase,
...
export function gearKinematics(inputDegrees: number, driverTeeth: number, drivenTeeth: number) {
  if (!Number.isFinite(inputDegrees) || ![driverTeeth,drivenTeeth].every(n => Number.isInteger(n) && n > 0)) throw new RangeError('Gear tooth counts must be positive integers');
  const ratio = drivenTeeth / driverTeeth;
  return { inputAngle: inputDegrees * Math.PI / 180, outputAngle: -inputDegrees * Math.PI / 180 / ratio, ratio, speedRatio: 1 / ratio, torqueRatio: ratio };
}

export function differentialKinematics(inputDegrees: number, turnRadius: number, direction: number, held = false) {
  if (![inputDegrees,turnRadius,direction].every(Number.isFinite) || turnRadius < 0 || ![-1,0,1].includes(direction)) throw new RangeError('Invalid differential input');
  const trackWidth = 1.6;
  const split = held ? 1 : turnRadius === 0 ? 0 : clamp(trackWidth / (2 * turnRadius), 0, 1) * direction;
  const carrier = inputDegrees * Math.PI / 180;
  return { carrier, left: carrier * (1 - split), right: carrier * (1 + split),
    leftRate: 1 - split, rightRate: 1 + split, pinionRelative: -split * carrier,
    split, trackWidth };
}
