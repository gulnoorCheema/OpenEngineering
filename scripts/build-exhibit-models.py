...
if ONLY is None or ONLY == 'sewing-machine':
...
if ONLY is None or ONLY == 'jet-engine':
 ...
 def rotor(name,radius,hub,blades,depth,material='Machined steel'):
  # Original swept, cambered teaching airfoils. No manufacturer blade profile.
  objects=[cyl(hub,depth*1.8,material='Dark iron',axis='X'),torus(hub,.018,material='Machined steel',axis='X')]
  N=7
  for i in range(blades):
   a=i*2*math.pi/blades; vv=[];ff=[]
   for side in [-1,1]:
    for j in range(N+1):
     t=j/N;r=hub+(radius-hub)*t
     for k in range(N+1):
      u=k/N;angle=a+(u-.5)*(.70-.47*t)-.23*t*t
      x=depth*(u-.5)+.13*t+.035*math.sin(u*math.pi)+side*.009*math.sin(math.pi*(.05+.9*u))
      vv.append((x,r*math.sin(angle),r*math.cos(angle)))
   layer=(N+1)**2
   for side in range(2):
    for j in range(N):
     for k in range(N):
      q=side*layer+j*(N+1)+k;face=(q,q+1,q+N+2,q+N+1)
      ff.append(face if side else face[::-1])
   border=list(range(N+1))+[j*(N+1)+N for j in range(1,N+1)]+[N*(N+1)+k for k in range(N-1,-1,-1)]+[j*(N+1) for j in range(N-1,0,-1)]
   for b,c in zip(border,border[1:]+border[:1]):ff.append((b,c,c+layer,b+layer))
   objects.append(mesh('Swept airfoil',vv,ff,material,0))
  join(name,objects)
 join('JetNacelle',[
  duct([(-3.08,1.45),(-3.02,1.51),(-2.9,1.59),(-2.7,1.62),(-1.8,1.6),(1.8,1.48),(2.8,1.32)]),
  torus(1.45,.065,(-3.07,0,0),'Machined steel','X'),
  *[duct([(x,r),(x+.045,r)],material='Machined steel') for x,r in [(-2.75,1.625),(-1.9,1.61),(-.5,1.57),(1,1.52),(2.4,1.39)]],
  *[cyl(.038,.055,(x,1.46*math.sin(a),-1.46*math.cos(a)),'Machined steel','X',6,.005) for x in [-2.5,-1.3,0,1.3,2.3] for a in [-math.pi*.615,math.pi*.615]]])
 ...
 # A tapered spinner loft, aligned to the fan's X-axis pivot.
 join('JetSpinner',[duct([(-.53,.018),(-.43,.12),(-.25,.22),(0,.27),(.1,.26)],arc=math.pi*2,material='Machined steel'),torus(.265,.018,material='Machined steel',axis='X')])
...
if ONLY is None or ONLY == 'mechanical-watch':
